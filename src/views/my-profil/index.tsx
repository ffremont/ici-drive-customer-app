      ...
      {this.state.email && (<form onSubmit={(e) => this.onSubmit(e)}>
        <Grid container spacing={2} alignContent="center" direction="column" alignItems="stretch" justify="center">
          ...
          <Grid item className="mp-field">
            <TextField variant="outlined" type="email" disabled onChange={(e) => this.setState({ email: e.target.value })} id="mp-email" inputProps={{ maxLength: 256, readOnly: true }} label="Email" fullWidth={true} value={this.state.email} />
          </Grid>
          ...
          <Grid item  className="mp-field">
            <TextField variant="filled" multiline rowsMax={2} type="text" onChange={(e) => this.setState({ address: e.target.value })} id="mp-address" inputProps={{ maxLength: 512 }} label="Adresse complète" fullWidth={true} value={this.state.address} />
          </Grid>

          <Grid item className="actions mp-field"> 
